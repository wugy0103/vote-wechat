...
// 加载配置文件  
var objConfig = JSON.parse(fs.readFileSync(path.join(__dirname, "logConfig.json") , "utf8"));
...
if (objConfig.appenders) {
    ...
    for (var i = 0, j = objConfig.appenders.length; i < j; i++) {
        ...
        if(process.platform === 'win32'){
            fileName =  __dirname[0] + ':' + fileName
        }
        // console.log(isAbsoluteDir(fileName))
...
helper.writeErr4Request = function (error, response,url) {
    var msg = "";
    if (url!=null) {
        msg = "请求地址为：" + url+", ";
    }
    if (error != null)
        ...
        msg += "请求API错误，状态码为：" + response.statusCode + ",错误信息：" + response.statusMessage;
    else msg += "未知错误";
    msg += "\r\n";
    logErr.error(msg);
};
